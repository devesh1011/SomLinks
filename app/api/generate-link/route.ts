...
const SYSTEM_PROMPT = `You are an assistant that creates Somnia EVM payment link metadata with beautiful dark theme designs.
Return ONLY a strict JSON object with these keys:
{
  "title": string,                 // short human title suitable for a link card
  "amount": number | null,         // positive number (STT) or null if not specified
...
- The design must prominently display the title, description, and amount in STT with clear visual hierarchy.
...
- Only set amount if explicitly mentioned in the user's prompt. If no amount is specified, set amount to null.
  ...
  try {
    ...
    if (!recipient || !prompt) {
      return NextResponse.json(
        { error: "recipient and prompt are required" },
        { status: 400 }
      );
    }
    ...
    if (!completionRes.ok) {
      ...
      return NextResponse.json(
        { error: `OpenAI error: ${completionRes.status} ${errText}` },
        { status: 502 }
      );
    }
    ...
    if (amount !== null && (!Number.isFinite(amount) || amount <= 0)) {
      return NextResponse.json(
        { error: "LLM returned invalid amount" },
        { status: 502 }
      );
    }
    ...
    const description = parsed.description
      ...
      : undefined;
    const componentHtml = parsed.componentHtml
      ? String(parsed.componentHtml)
      : undefined;
